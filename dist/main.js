...
var require_async = __commonJS({
  "node_modules/resolve/lib/async.js"(exports2, module2) {
    ...
    var defaultIsDir = function isDirectory(dir2, cb) {
      fs2.stat(dir2, function(err, stat) {
        if (!err) {
    ...
    module2.exports = function resolve2(x, options, callback) {
      ...
      function loadAsFile(x2, thePackage, callback2) {
        ...
        function load(exts2, x3, loadPackage) {
          ...
          function onpkg(err2, pkg_, dir2) {
            ...
            if (dir2 && pkg && opts.pathFilter) {
              var rfile = path3.relative(dir2, file);
              var rel = rfile.slice(0, rfile.length - exts2[0].length);
              var r = opts.pathFilter(pkg, x3, rel);
              if (r) return load(
                [""].concat(extensions.slice()),
                path3.resolve(dir2, r),
                pkg
              );
            }
            isFile(file, onex);
      ...
      function loadpkg(dir2, cb2) {
        if (dir2 === "" || dir2 === "/") return cb2(null);
        if (process.platform === "win32" && windowsDriveRegex.test(dir2)) {
          return cb2(null);
        }
        if (nodeModulesRegex.test(dir2)) return cb2(null);
        maybeRealpath(realpath, dir2, opts, function(unwrapErr, pkgdir) {
          if (unwrapErr) return loadpkg(path3.dirname(dir2), cb2);
          var pkgfile = path3.join(pkgdir, "package.json");
          isFile(pkgfile, function(err2, ex) {
            if (!ex) return loadpkg(path3.dirname(dir2), cb2);
            ...
            readPackage(readFile, pkgfile, function(err3, pkgParam) {
              ...
              if (pkg && pkg.main) {
                ...
                loadAsFile(path3.resolve(x2, pkg.main), pkg, function(err4, m, pkg2) {
                  ...
                  var dir2 = path3.resolve(x2, pkg2.main);
                  loadAsDirectory(dir2, pkg2, function(err5, n, pkg3) {
      ...
      function processDirs(cb2, dirs) {
        ...
        var dir2 = dirs[0];
        isDirectory(path3.dirname(dir2), isdir);
...
var require_sync = __commonJS({
  "node_modules/resolve/lib/sync.js"(exports2, module2) {
    ...
    var defaultIsDir = function isDirectory(dir2) {
      try {
        var stat = fs2.statSync(dir2, { throwIfNoEntry: false });
      } catch (e) {
        if (e && (e.code === "ENOENT" || e.code === "ENOTDIR")) return false;
        throw e;
      }
    ...
    module2.exports = function resolveSync(x, options) {
      ...
      function loadpkg(dir2) {
        ...
        if (nodeModulesRegex.test(dir2)) return;
        var pkgfile = path3.join(maybeRealpathSync(realpathSync, dir2, opts), "package.json");
        if (!isFile(pkgfile)) {
          return loadpkg(path3.dirname(dir2));
        }
      ...
      function loadNodeModulesSync(x2, start) {
        ...
        for (var i = 0; i < dirs.length; i++) {
          var dir2 = dirs[i];
          if (isDirectory(path3.dirname(dir2))) {
            var m2 = loadAsFileSync(dir2);
            if (m2) return m2;
            var n2 = loadAsDirectorySync(dir2);
            if (n2) return n2;
          }
        }
      }
  ...
  try {
    ...
    if (cookies.length) {
      ...
      for (const cookie of cookies) {
        ...
        cookiesHeader.push((0, import_cookie.serialize)(name, value));
      }
...
var ReadableServerResponse = class extends import_node_http.ServerResponse {
  constructor(req) {
    super(req);
    this.responsePromise = new Promise((resolve2, reject) => {
...
function createHandler({
  ...
  (0, import_node_assert.default)(dir2, "dir is required");
  (0, import_node_assert.default)(protocol2, "protocol is required");
  (0, import_node_assert.default)(hostname, "hostname is required");
  (0, import_node_assert.default)(port, "port is required");
  dir2 = dev2 ? process.cwd() : dir2;
  if (debug) {
    console.log("Next.js handler", { dev: dev2, dir: dir2, hostname, port, debug });
  }
  ...
  if (dev2) {
    const server = require(import_resolve.default.sync("next/dist/server/lib/start-server", { basedir: dir2 }));
    const preparePromise2 = server.startServer(serverOptions);
    return {
      localhostUrl,
      createInterceptor: async ({ session: session2 }) => {
        (0, import_node_assert.default)(session2, "Session is required");
        await preparePromise2;
        if (debug)
          console.log(
  ...
  const next = require(import_resolve.default.sync("next", { basedir: dir2 }));
  const config = require(import_node_path.default.join(dir2, ".next", "required-server-files.json")).config;
  process.env.__NEXT_PRIVATE_STANDALONE_CONFIG = JSON.stringify({
    ...config,
    ...nextOptions == null ? void 0 : nextOptions.conf
  });
  const app2 = next(serverOptions);
  const handler = app2.getRequestHandler();
  const preparePromise = app2.prepare().catch((err) => {
  ...
  async function createInterceptor({ session: session2 }) {
    (0, import_node_assert.default)(dir2, "dir is required");
    (0, import_node_assert.default)(session2, "Session is required");
    (0, import_node_assert.default)(import_node_fs.default.existsSync(dir2), "dir does not exist");
    if (debug)
      console.log(
        `Server Intercept Enabled, ${localhostUrl} will be intercepted by ${dir2}`
      );
    ...
    protocol2.handle("http", async (request) => {
      try {
        (0, import_node_assert.default)(
          request.url.startsWith(localhostUrl),
          "External HTTP not supported, use HTTPS"
        );
        const req = await createRequest({ socket, request, session: session2 });
        const res = new ReadableServerResponse(req);
        if (!req.url) throw new Error("Request URL is empty");
        const url = (0, import_node_url.parse)(req.url, true);
        handler(req, res, url);
        const response = await res.getResponse();
        try {
          const cookies = (0, import_set_cookie_parser.parse)(
            response.headers.getSetCookie().reduce((r, c) => {
              return [...r, ...(0, import_set_cookie_parser.splitCookiesString)(c)];
            }, [])
          );
          for (const cookie of cookies) {
            const {
            ...
            const expirationDate = expires ? expires.getTime() : maxAge ? Date.now() + maxAge * 1e3 : void 0;
            if (expirationDate && expirationDate < Date.now()) {
              await session2.cookies.remove(request.url, cookie.name);
              continue;
            }
...
var dev = true;
var dir = import_path.default.join(appPath, ".next", "standalone");
...
var createWindow = async () => {
  ...
  if (dev) {
    ...
    mainWindow.on("closed", () => {
      mainWindow = null;
      stopIntercept == null ? void 0 : stopIntercept();
    });
...
import_electron.app.on(
  "activate",
  () => import_electron.BrowserWindow.getAllWindows().length === 0 && !mainWindow && createWindow()
);
